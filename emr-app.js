...
function _showNIDLinkDialog(bookingKey, booking, bookingName, startVisit) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--panel);border:1px solid var(--border);border-radius:22px;
      ...
      if (bnid) return `المريض <b>${sanitize(bookingName)}</b> يحمل الرقم <b>${bnid}</b> ولكن ليس له ملف سابق.`;
      return `الحجز لـ <b>${sanitize(bookingName)}</b> لا يحتوي على رقم وطني مؤكد.`;
    })()}
        </div>
      </div>
      ...
      <div style="display:flex;gap:8px">
        <button onclick="_nldOpenNew('${bookingKey}',${startVisit})"
          style="flex:1;background:var(--green);border:none;color:#fff;
                 padding:12px;border-radius:10px;font-family:'Tajawal',sans-serif;font-weight:800;cursor:pointer;
                 box-shadow:0 8px 20px rgba(16,185,129,0.3); font-size: 1.05rem;">
          <i class="fas fa-user-plus"></i> تأكيد فتح ملف طبي جديد
        </button>
...
function refreshPatientFileUI(uid) {
  ...
  if (container) {
    ...
    if (activeEmrTab === 'dental-chart-tab' && window.DentalChartModule && typeof window.DentalChartModule.render === 'function') {
      ...
      _restoreScroll(mainDiv, scrollPos);
    } else if (activeEmrTab === 'cardio-tab' && window.CardioModule && typeof window.CardioModule.render === 'function') {
      window.CardioModule.render('_cardioContainer', uid);
      _restoreScroll(mainDiv, scrollPos);
    } else {
...
function _restoreScroll(mainDiv, scrollPos) {
  let attempts = 0;
...
async function safeViewPatientFile(phoneOrUid) {
  ...
  if (!_patients[phoneOrUid] && window._pager) {
    const loaded = await window._pager.getPatient(phoneOrUid);
...
function switchEmrTab(tabId) {
  ...
  if (tabId === 'timeline-tab') {
    ...
    const el = document.getElementById('emr-tab-growth-chart');
    if (el) { el.style.display = 'block'; el.classList.add('active-content'); }
    if (window.GrowthChartModule && typeof window.GrowthChartModule.render === 'function') {
      const pid = window.activePatientId || (window.EMRContext && window.EMRContext.activePatientId) || null;
      window.GrowthChartModule.render('_growthChartContainer', pid);
    } else if (window.ArgonSpecialtyLoader && typeof window.ArgonSpecialtyLoader.loadModule === 'function') {
      window.ArgonSpecialtyLoader.loadModule('growth_chart_module');
      setTimeout(function () {
        if (window.GrowthChartModule) {
          const pid = window.activePatientId || (window.EMRContext && window.EMRContext.activePatientId) || null;
          window.GrowthChartModule.render('_growthChartContainer', pid);
        }
      }, 1000);
    }
  } else if (tabId === 'cardio-tab') {
    document.querySelectorAll('.emr-tab-btn').forEach(b => {
      if (b.textContent.includes('القلب')) b.classList.add('active');
    });
    const el = document.getElementById('emr-tab-cardio');
    if (el) { el.style.display = 'block'; el.classList.add('active-content'); }
    if (window.CardioModule && typeof window.CardioModule.render === 'function') {
      const pid = window.activePatientId || (window.EMRContext && window.EMRContext.activePatientId) || null;
      window.CardioModule.render('_cardioContainer', pid);
    } else if (window.ArgonSpecialtyLoader && typeof window.ArgonSpecialtyLoader.loadModule === 'function') {
      window.ArgonSpecialtyLoader.loadModule('cardio_module');
      setTimeout(function () {
        if (window.CardioModule) {
          const pid = window.activePatientId || (window.EMRContext && window.EMRContext.activePatientId) || null;
          window.CardioModule.render('_cardioContainer', pid);
        }
      }, 1000);
